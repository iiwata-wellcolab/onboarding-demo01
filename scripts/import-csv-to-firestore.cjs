...
// Firebaseの初期化
const serviceAccount = require('../firebase-credentials.json'); // 認証情報ファイルへのパス
admin.initializeApp({
    ...
    fs.createReadStream(path.join(__dirname, '../temp/org_master.csv'))
      ...
      .on('end', async () => {
        ...
        for (const org of organizations) {
          // 空のdivision_codeをスキップ
          if (!org.division_code || org.division_code.trim() === '') {
            console.log('警告: 空のdivision_codeをスキップします');
            continue;
          }
      ...
      .on('end', async () => {
        ...
        for (const emp of employees) {
          ...
          if (!emp.emp_no || emp.emp_no.trim() === '') {
            console.log('警告: 空のemp_noをスキップします');
            continue;
          }
